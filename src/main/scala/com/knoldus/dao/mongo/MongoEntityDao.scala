package com.knoldus.dao.mongo

import com.knoldus.dao.EntityDao
import com.knoldus.dao.commons.WithId
import com.knoldus.dao.exceptions.UnknownSortingFieldException
import com.knoldus.dao.filters._
import com.knoldus.dao.sorting.Direction.{ Ascending, Descending }
import com.knoldus.dao.sorting.{ Field, SortBy }
...
trait MongoEntityDao[T] extends EntityDao[T] {
  ...
  override def get(filter: Filter)(implicit ec: ExecutionContext): Future[Option[WithId[T]]] = {
    val result = executeWithPredicate(
      filter,
      predicate =>
        for {
  ...
  final protected def buildSorting(sortBy: SortBy): Try[Bson] =
    Try {
  ...
  final protected def documentToWithId(document: Document): Try[WithId[T]] =
    for {
  ...
  override def count(filter: Filter)(implicit ec: ExecutionContext): Future[Int] =
    executeWithPredicate(
      filter,
      predicate => collection.countDocuments(predicate).toFuture().map(_.toInt)
    )

  final protected def collection: MongoCollection[Document] =
  ...
  override def updateMany(filter: Filter, updater: T => T)(implicit ec: ExecutionContext): Future[Int] = {
    ...
    def executeUpdates(currentWithIds: Seq[WithId[T]]): Future[Int] = {
      val entityUpdates = currentWithIds.map {
        case WithId(currentEntity, id) =>
          val updatedEntity = updater(currentEntity)
          val fieldUpdates = entityToDocument(updatedEntity).map {
            case (name, value) =>
              set(name, value)
          }.toSeq
  ...
  override def deleteMany(filter: Filter)(implicit ec: ExecutionContext): Future[Int] =
    executeWithPredicate(
      filter,
      predicate => collection.deleteMany(predicate).toFuture().map(_.getDeletedCount.toInt)
    )
  ...
  final protected def basicFilterMapper: PartialFunction[Filter, Try[Predicate]] = {
    case And(left, right) =>
      for {
